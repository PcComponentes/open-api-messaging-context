...
{
    /** @test */
    public function given_valid_schema_when_parse_v3_from_request_then_ensure_parsed_schema_is_correct(): void
    {
        $allSpec = Yaml::parse(\file_get_contents(__DIR__ . '/valid-openapi-v3-spec.yaml'));
        $path = '/url/endpoint';
        $method = 'post';
        $contentType = 'application/json';
        $schema = (new OpenApiSchemaParser($allSpec))->fromRequest($path, $method, $contentType);
